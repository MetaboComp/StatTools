...
sampling_from_distribution<-function(X,
  ...
  if(is.numeric(X)){
  if(missing(extend)){extend=0.05*(max(X)-min(X))}  
  from=min(X)-extend
  to=max(X)+extend
  if(missing(upperlimit)){upperlimit=to}
  if(missing(lowerlimit)){lowerlimit=from}
  if(upperlimit-lowerlimit<=0){stop("Dude, upperlimit should be higher than lower limit")}

  if(from<lowerlimit){from=lowerlimit}
  if(to>lowerlimit){to=upperlimit}
  denss = density(
    x = X,
    from = from,
    to = to,
